...
import type { Recipe, RecipeIngredientDetail } from '@shared/schema';
...
export default function RecipeDetail() {
  ...
  const { data: recipe, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await recipeAPI.getById(parseInt(id));
              ...
              <CardContent>
                <ul className="space-y-2">
                  {Array.isArray(recipe.ingredients) && recipe.ingredients.length > 0 ? (
                    recipe.ingredients.map((ingredient: RecipeIngredientDetail, index: number) => (
              ...
              <Card className="mt-6">
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    {recipe.hashtags.map((tag: string, index: number) => (
                      <span 
                        key={index}
